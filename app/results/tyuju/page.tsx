...
import { Award, CheckCircle, ChevronDown, ArrowLeft } from 'lucide-react';
...
const resultsByYear: Record<2026 | 2025 | 2024 | 2023, SchoolCategory> = {
    2026: {
        boys: ['海陽', '立教新座', '成城', '高輪', 'サレジオ学院', '聖光学院', '鎌倉学園', '逗子開成', '攻玉社'],
        girls: ['淑徳与野', '捜真女学校', '不二聖心女子学院', 'フェリス女学院', '湘南白百合', '聖セシリア女子', '東洋英和', '清泉女学院', '横浜共立', '鎌倉女学院'],
        coed: ['栄東', '埼玉栄', '宮崎日大', '佐久長聖', '明大世田谷', '山手学院', '関東学院', '神奈川大学附属', '東京大学附属', '横浜創英', '青陵', '桐蔭', '桐光', '品川翔英', '三田国際科学学園（IC）', '日大日吉', '横須賀学院', '青山学院横浜英和', '横浜市立南高等学校附属']
    },
    2025: {
...
const testimonials: Testimonial[] = [
    {
        ...
        school: '聖光学院',
        grade: '精華小学校',
        studentVoice: `僕は6年の志望校特訓が始まり、算数が本格的に難しくなりました。そのため一人で解決するのが困難になりました。そこで個別で教えてくれるLEFYに入りました。友達がいたことや、志望校の聖光学院出身の先生がいたこともきっかけになりました。

しかし、個別と聞くと僕は今まで少し嫌いでした。これは、つまらない印象があったからです。けれど、LEFYには話しやすい先生がいたため、楽しく授業を受けることができました。

授業では、過去問の解き直しが早めに終わってしまうこともありました。そんな時は志望校の古い問題や他の学校の面白い問題を出してくれて楽しかったです。

さらに、点数が取れなかった時でも「君なら、できないわけないんだ」など励ましてくれ、あまり落ち込むことがありませんでした。みんなできない問題や、できないといけない問題を教えてくれ、とても分かりやすかったです。

最初は合格できるか不安だった聖光学院も、山本先生のおかげで無事合格することができました。
本当にありがとうございました。`,
        parentVoice: `大手塾の志望校特訓が始まり、算数の問題が難しくなったことで、家庭での復習に多くの時間を取られるようになりました。その結果、他の教科の学習が十分に回らなくなってしまいました。
質問教室で先生に聞きに行くのも嫌がるので、個別指導を検討しておりました。知り合いの方から、以前山本先生に教わり、楽しく授業を受けられていたことを聞いていたので、入塾を決めました。

入塾してからは、大手塾の問題解説をしていただく他、先生の方で志望校の古い過去問から無理がない範囲で進めていただきました。算数以外にも、社会や理科の学習方法もご指導いただけたおかげで、バランスよく学習を進めていくことができました。過去問で思うように点数が取れず不安になった時も、先生が励ましてくださり、前向きな気持ちで最後まで諦めずに入試に立ち向かうことができました。

第一志望校に合格できたのは、先生のご指導と温かい支えのおかげです。本当にありがとうございました。`
    },
    {
        id: 'tk',
        studentName: 'T.K君',
...
中学受験は終わりましたが、学ぶことはこれで終わりではないので今後もご指導いただきたいと考えております。 本当にありがとうございました。`
    },
    {
        ...
        school: '南附属・フェリス女学院',
        grade: '横浜市立N小学校',
        studentVoice: `小学6年生の10月にLEFYに入塾しました。
その頃の私は、もともと得意だったはずの算数で、SAPIXや四谷大塚などの公開模試の点数が落ち込み、原因がよくわからず悩んでいました。
国語は文章を表面的にしか読めず、社会は知識が抜けていて、理科は計算問題をポロポロ落としてしまうような状態でした。

山本先生は、そんな私のつまずきをすぐに見抜き、「算数の"型"が身についていない。本番までに一つでも多く型を覚えよう」
と声をかけてくださいました。
確かに私は、基礎に立ち返らず我流で解き進めてしまうクセがありました。

そこからは、先生が指定してくださった問題集を使って、とにかく型を覚えることに集中しました。
とはいえ、すぐに模試の結果が上がるわけではなく、SAPIXの学校別模試で算数偏差値28という結果が出たときは本当にショックで
「ああ、もう終わったな。受かるわけないな」と思いました。

しかし山本先生は、どんなに結果が悪くても「実力はあるんだし、たまたまだよ」と励ましてくださり、
根気強く型を教え、勉強の戦略を一緒に考えてくださいました。
国語の宮本先生は記述の書き方を丁寧に教えてくださいました。
社会では石川先生が、間違えたところや苦手な単元をもとに、並び替え問題や一問一答を作ってくださいました。
理科の和田先生とは計算問題をたくさん解き、安定して高得点が取れるようになりました。

公開模試は最後までパッとしませんでしたが、1月に入り学校を休んで勉強に集中するようになってからは
必死に算数の型を覚え続け、他の教科もやり込みました。

そして迎えた受験当日。
試験を受けているときに、「あ、これ受かったな」と思いました。
LEFYで教えてもらったことを思い出しながら、落ち着いて問題を解くことができました。
実力のピークを試験当日に持ってこれたのだと思います。

こんな私を最後まで見守ってくださった先生方、本当にありがとうございました。

受験を通して、「基礎」の大切さを強く実感しました。
受験生の皆さん、頑張ってください！`,
        parentVoice: `娘は、テキトーなくせに頑固で、自分のやり方にこだわり、
「こうしたほうがいいんじゃない？」というアドバイスは、「へーい」と言いながら華麗にスルーするタイプです。

よく"中学受験に向いている子どもの特徴"として「素直な性格」が挙げられますが、娘はその真逆。
親からすると「なんでそんな非効率な道を選ぶの？めんどくさ！」と、毎回イライラさせられっぱなしでした。
当然、6年生になって成績は下降。
しかし、今までの悪い積み重ねを急に修正できるはずもなく、
秋になって「さあ、いよいよどうしよう」というタイミングでLEFYに出会いました。

LEFYの体験授業では、4教科すべての先生とウマが合ったようで、本人は楽しそうに通い始めました。

山本先生は娘の性格を完全に理解し、「ここがダメ」「ここは強み」と、本人にわかるように説明してくれました。
親が言うと100％反発するのに、先生が言うとなぜか素直に聞くという不思議。

1月、算数の「型」を猛烈に覚えこんでいるとき、娘が「あぁ、これをもっと早いうちにやっていたらよかったのになぁ」と言った瞬間、
私は心の中で「気付くのおっそ！」と叫びつつも、「まあ、今気付いただけマシか…」と自分をなだめました。
もし入試までに覚えこみが間に合わなかったら、それもこの子の人生だと腹をくくり、親としては見守ることにしました。

さらに娘は、頑固でテキトーなくせに、実はメンタルが豆腐。
ここも山本先生が絶妙にかじ取りしてくださり、私なら絶対に言えない「当日は7割の力で臨むように。楽しんで！」
という魔法の言葉をかけてくれました。
この"7割"が娘にはドンピシャで、頑張りすぎなくていいという安心感につながったのだと思います。

入試本番、娘は本当に楽しそうな顔で試験会場から出てきて、その表情を見ただけで
「この子の中ではやり切ったんだな」と感じ、親としてとても嬉しかったです。
山本先生をはじめ、ほかの教科の先生方も、とても優しく、面白く授業を進めてくださり
娘にとって幸せな時間だったと思います。
...
export default function ResultsJukenPage() {
